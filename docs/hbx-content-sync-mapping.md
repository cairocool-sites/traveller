...
HBX hotel images are stored as supplier image paths only, for example `00/001001/001001a_hb_a_001.jpg`. Public rendering builds the display URL from the official HBX photo CDN pattern `https://photos.hotelbeds.com/giata/{size}/{path}`. The platform does not download, proxy, or commit HBX image binaries. `visualOrder = 0` is treated as the primary image, and `order` is used for secondary sorting within the supplier image group.
