...
class SupplierManager
{
    public function resolve(string $supplierCode, ?SupplierOperation $operation = null): HotelSupplierInterface
    {
        ...
        return match ($supplier->integration_type) {
            ...
            SupplierIntegrationType::Rest => match ($supplier->code) {
                'tbo_hotels' => app(TboHotelSupplier::class, ['supplier' => $supplier]),
                'ratehawk_hotels' => app(RateHawkHotelSupplier::class, ['supplier' => $supplier]),
                default => throw new UnsupportedSupplierOperationException('No REST adapter is registered for this supplier.'),
            },
